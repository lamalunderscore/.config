...
class NormRegression(MultiLinearRegression):
    ...
    def train(self, data: np.ndarray, target: np.ndarray):
        ...
        if self.random_0_to_1:
            ...
            w = np.random.uniform(-1, 1, data.shape[0] + 1)
        self.intercept = w[0]
        self.slope = w[1:]
        ...
        for i in range(self.iterations):
            ...
            w = w - self.learning_rate * self._gradient(w, data)
...
class LassoRegression(NormRegression):
    ...
    def _gradient(
        weights: np.ndarray, data: np.ndarray, prediction:np.ndarray, target: np.ndarray
    ) -> np.ndarray:
        (
            gradient = (2 / len(prediction) * np.dot(data.T, target.flatten() - prediction)
            + 2 * factor * weights)
            return gradient
        )
