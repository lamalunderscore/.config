...
class MultiLinearRegression:
    ...
    def set_params(self, parameters:np.ndarray) -> None:
        ...
        :param parameters:np.ndarray: array of all coefficients to be set (intercept, slope)
        """
    ...
    def _set_slope(self, slope:np.ndarray) -> None:
        ...
        :param slope:np.ndarray: slope coefficients to be set
        """
        self._slope = slope
        return
    ...
    def get_params(self) -> np.ndarray:
        ...
        :returns:np.ndarray: all coefficients (intercept, slope)
        """
    ...
    def get_slope(self) -> np.ndarray:
        ...
        :returns:np.ndarray: slope coefficients
        """
        return self._slope
    ...
    def train(self, X:pd.DataFrame, y:pd.DataFrame) -> None:
        ...
        if len(X) != len(model.get_slope()):
             raise ValueError("The number of features does not fit the number of model coefficients.")
        ...
        self._set_intercept(float(parameters[0]))
    ...
    def predict(self, X:pd.DataFrame) -> None:
        ...
        if type(slope) == int:
            raise ValueError("The model has not been trained yet, and can therefore not predict any data.")
