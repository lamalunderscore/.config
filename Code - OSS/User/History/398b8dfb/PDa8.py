...
class MultiLinearRegression:
    def __init__(
        self, default_slope: np.ndarray = None, default_intercept: float = None
    ):
        """
        This initializes a class and sets necessary check values.

        :param default_slope:numpy.ndarray: default
        slope to use without training
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        in the data and the number of coefficients
        of the model are not the same
        ...
        if type(slope) == None:
            raise ValueError(
                "The model does not have the slope parameter set yet. Please set it when initializing, or train the model first."
            )

        if type(intercept) == None:
            raise ValueError(
                "The model does not have the intercept parameter set yet. Please set it when initializing, or train the model first."
            )
        ...
        if X.shape[1] != len(slope):
            raise ValueError(
                ...
                X.shape[1],
                ") does not fit the number of model coefficients (",
                len(slope),
                ").",
            )
