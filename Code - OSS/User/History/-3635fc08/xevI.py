# Authors: Mikolaj Pawlak (s5055547) and Felix Michalak(s5142644)
...
def range_phi(phi: float) -> float:
    ...
    if phi <= -cmath.pi:
        return range_phi(phi + 2 * cmath.pi)
    if cmath.pi < phi:
        return range_phi(phi - 2 * cmath.pi)
    return phi
...
def combine_fir(h1: list, h2: list) -> list:
    h = list()
    for n in range(max(len(h1), len(h2))):
        try:
            a = h1[n]
        except IndexError:
            a = 0
...
if __name__ == "__main__":
    ...
    print(output_signal(amplitude_x, phi_x, radian, h))
