...
class RegressionPlotter:
    def __init__(self, data: pd.DataFrame, target: pd.DataFrame) -> None:
        """
        This initialies data points for
        this instance to plot as a scatterplot
        ...
        if len(self._target) != len(self._data):
            raise AttributeError(
                "Data and target do not share the same sample size."
            )

    def plot(self, model: object) -> None:
        """
        This function plots a regression line
        or plane depending on its number of coefficients.

        :param model:object: model to plot
        :raises AttributeError: if model does not suppot
                                model.get_intercept() or model.get_slope()
        """
        try:
            ...
            print(
                "Specified model does not support necessary getter functions."
            )
    ...
    def plot_all(self, model: object) -> None:
        """
        This function is used to generate
        subplots for each coefficient of a given model.

        :model:object: Model to plot
        :raises AttributeError: if model does not
        suppot model.get_intercept() or model.get_slope()
        """
        try:
            slope = model.get_slope()
            intercept = model.get_intercept()
        ...
        for i in range(num_coefficients):
            ax = plt.subplot(
                int(np.sqrt(num_coefficients)) + 1,
                int(np.sqrt(num_coefficients)) + 1,
                i + 1,
            )
            plt.axline(
